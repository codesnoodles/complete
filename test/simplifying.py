    ...
    for i, _ in enumerate(gpt2_model.transformer.h):
        ...
        if new_cfg.tie_valproj_init is not None:
            ...
            if new_cfg.tie_valproj_init == "last_block":
                ...
                new_block.attn.c_proj.id.data = new_block.attn.v_attn.id.data.clone(
                ).T
            else:
...
class myGPT2Block(nn.Module):
    ...
    def __init__(self, config, layer_idx=None):
        ...
        if config.norm_type == "ln":
            ...
            self.ln_1 = (nn.Identity() if layer_idx > 0 else nn.LayerNorm(
...
class myGPT2Attention(nn.Module):
    ...
    def __init__(self, config, is_cross_attention=False, layer_idx=None):
        ...
        if (config.value_skip_gain != 1 or value_resid_gain != 0
        ...
        if config.val_proj_init_std is not None:
            self.v_attn.weight.data.normal_(mean=0.0,
                                            std=config.val_proj_init_std)
            self.c_proj.weight.data.normal_(mean=0.0,
                                            std=config.val_proj_init_std)
...
class MyConv1D(nn.Module):
    ...
    def __init__(
        ...
        if skip_gain is None:
            ...
            assert nx == nf
            self.resid_gain = nn.Parameter(torch.Tensor([resid_gain]),
            ...
            if init_type == "orth":
                ...
                self.id = nn.init.normal_(torch.empty(nx, nx),
                                          std=1 / math.sqrt(nx)).cuda()
            else:
                raise NotImplementedError
            self.skip = True
