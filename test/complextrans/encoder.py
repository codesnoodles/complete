...
class EncoderBlock_dpt(nn.Module):

    def __init__(self, in_channels, out_channels, dense_depth=4):
        super(EncoderBlock_dpt, self).__init__()
        self.conv1 = BasicConv(in_channels=in_channels,
                               out_channels=out_channels,
                               kernel_size=1)
        self.dense = DenseBlock(depth=dense_depth, in_channels=out_channels)
...
class EncoderBlock_Lpa(nn.Module):

    def __init__(self, in_channels, out_channels, dense_depth=4):
        super(EncoderBlock_Lpa, self).__init__()
        self.conv1 = BasicConv(in_channels=in_channels,
                               out_channels=out_channels,
                               kernel_size=1)
        self.dense = DenseBlock_Lpa(depth=dense_depth,
...
class EncoderBlock_LpaDilate(nn.Module):

    def __init__(self, in_channels, out_channels, dense_depth=4):
        super(EncoderBlock_LpaDilate, self).__init__()
        self.conv1 = BasicConv(in_channels=in_channels,
                               out_channels=out_channels,
                               kernel_size=1)
        self.dense = DenseBlock_LpaDilate(depth=dense_depth,
                                          in_channels=out_channels)
        self.conv2 = BasicConv(in_channels=out_channels,
                               out_channels=out_channels,
                               kernel_size=(1, 3),
...
class Encoder_LpaGate(nn.Module):

    def __init__(self, in_channels, out_channels, dense_depth=4):
        super(Encoder_LpaGate, self).__init__()
        self.conv1 = BasicConv(in_channels=in_channels,
                               out_channels=out_channels,
                               kernel_size=1)
        self.dense = Dense_LpaGate(depth=dense_depth, in_channels=out_channels)
        self.conv2 = BasicConv(in_channels=out_channels,
                               out_channels=out_channels,
                               kernel_size=(1, 3),
                               stride=(1, 2),
...
class EncoderBlock_depth_Lpa(nn.Module):

    def __init__(self, in_channels, out_channels, dense_depth=4):
        super(EncoderBlock_depth_Lpa, self).__init__()
        self.conv1 = BasicConv(in_channels=in_channels,
                               out_channels=out_channels,
                               kernel_size=1)
        self.dense = DenseBlock_depth_Lpa(in_channels=out_channels,
                                          groups=out_channels,
                                          depth=dense_depth)
        self.conv2 = BasicConv(in_channels=out_channels,
                               out_channels=out_channels,
