...
class DenseBlock_depth_Lpa(nn.Module):
    ...
    def __init__(self,
                 ...
                 t_length=126,
                 num_features=257):
        super(DenseBlock_depth_Lpa, self).__init__()
        self.depth = depth
        self.in_channels = in_channels
        self.pad = nn.ConstantPad2d((1, 1, 1, 0), value=0.)
        self.twidth = 2
    ...
    def forward(self, x):
        ...
        for i in range(self.depth):
            out = getattr(self, 'pad{}'.format(i + 1))(skip)
            out = getattr(self, 'conv{}'.format(i + 1))(out)
            out = getattr(self, 'flpa{}'.format(i + 1))(out)
...
class DenseBlock_Lpa(nn.Module):
    ...
    def __init__(self, depth=4, in_channels=64, t_length=32, num_features=257):
...
class DenseBlock_LpaDilate(nn.Module):

    def __init__(self, depth=4, in_channels=64, t_length=64, num_features=257):
        super(DenseBlock_LpaDilate, self).__init__()
        self.depth = depth
        self.in_channels = in_channels
        self.pad = nn.ConstantPad2d((1, 1, 1, 0), value=0.)
        self.twidth = 2
...
class Dense_LpaGate(nn.Module):

    def __init__(self, depth=4, in_channels=64, t_length=32, num_features=257):
        super(Dense_LpaGate, self).__init__()
        self.depth = depth
        self.in_channels = in_channels
        self.pad = nn.ConstantPad2d((1, 1, 1, 0), value=0.)
        self.twidth = 2
