...
class ConnectConformer_lpa(nn.Module):

    def __init__(self, in_channels, out_channels, num_layers=4):
        super(ConnectConformer_lpa, self).__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.mid_channels = in_channels // 2
        self.input = nn.Sequential(
        ...
        self.row_norm = nn.ModuleList([])
        self.col_norm = nn.ModuleList([])
        self.tpa = nn.ModuleList([])
        self.fpa = nn.ModuleList([])
        ...
        for i in range(num_layers):
            ...
            self.tpa.append(
                LocalpatchAttention(in_channels=self.mid_channels,
                                    reduction=1,
                                    pool_t=1,
                                    pool_f=129,
                                    add_input=True))
            self.fpa.append(
                LocalpatchAttention(in_channels=self.mid_channels,
                                    ...
                                    pool_f=1,
                                    add_input=True))
            self.attn_norm.append(nn.GroupNorm(1, self.mid_channels, eps=1e-8))
        self.output = nn.Sequential(
            nn.PReLU(), nn.Conv2d(self.mid_channels, out_channels, 1))
    ...
    def forward(self,input):
        ...
        for i in range(len(self.row_trans)):
            ...
            # attn
            output = self.ca[i](output)
            output = self.tpa[i](output)
            output = self.fpa[i](output)
            output = self.attn_norm[i](output)
        del row_input, row_output, col_input, col_output  # type:ignore
        output = self.output(output)
        return output


class ConnectConformer_dual(nn.Module):
    """
    use the tpa and fpa
    """

    def __init__(self, in_channels, out_channels, num_layers=4):
        super(ConnectConformer_dual, self).__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.mid_channels = in_channels // 2
...
class CrossConformer_big_change(nn.Module):
    ...
    def forward(self, x):
        ...
        x_t = self.tc1(x_t)
        x_t = x_t.view(b, f, t, -1).permute(0, 3, 2, 1).contiguous()
        x_t = self.norm1(x_t) + x1
        # x_t = self.norm1(x_t)

        x_f = x.permute(0, 2, 3, 1).contiguous().view(b * t, f, -1)
        x_f = self.fc1(x_f)
        x_f = x_f.view(b, t, f, -1).permute(0, 3, 1, 2).contiguous()
        x_f = self.norm2(x_f) + x1
        # x_f = self.norm2(x_f)
        ...
        y_t = self.tc2(y_t)
        y_t = y_t.view(b, f, t, -1).permute(0, 3, 2, 1).contiguous()
        y_t = self.norm3(y_t) + y1
        # y_t = self.norm3(y_t)
        y_f = y_f.permute(0, 2, 3, 1).contiguous().view(b * t, f, -1)
        y_f = self.fc2(y_f)
        y_f = y_f.view(b, t, f, -1).permute(0, 3, 1, 2).contiguous()
        y_f = self.norm4(y_f) + y2
        # y_f = self.norm4(y_f)
        ...
        return output
class CrossConformer_big_changeDilate(nn.Module):

    def __init__(self, in_channels, out_channels):
        super(CrossConformer_big_changeDilate, self).__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.mid_channels = in_channels // 2
        self.input = nn.Sequential(
...
class CrossConformer_big_change_dual(nn.Module):

    def __init__(self, in_channels, out_channels):
        super(CrossConformer_big_change_dual, self).__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.mid_channels = in_channels // 2
        self.input = nn.Sequential(
            nn.Conv2d(in_channels=in_channels,
                      ...
                      kernel_size=1,
                      stride=1), nn.PReLU())
        self.tc1 = ConformerBlock_residual(
            dim=self.mid_channels,
            dim_head=self.mid_channels // 4,
            conv_kernel_size=self.mid_channels // 2 - 1,
            attn_dropout=0.2,
            ff_dropout=0.2)
        self.tc2 = ConformerBlock_residual(
            dim=self.mid_channels,
            dim_head=self.mid_channels // 4,
            conv_kernel_size=self.mid_channels // 2 - 1,
            attn_dropout=0.2,
            ff_dropout=0.2)
        self.fc1 = ConformerBlock_residual(
            dim=self.mid_channels,
            dim_head=self.mid_channels // 4,
            conv_kernel_size=self.mid_channels // 2 - 1,
            attn_dropout=0.2,
            ff_dropout=0.2)
        self.fc2 = ConformerBlock_residual(
            dim=self.mid_channels,
            dim_head=self.mid_channels // 4,
            conv_kernel_size=self.mid_channels // 2 - 1,
            attn_dropout=0.2,
        ...
        self.output = nn.Sequential(nn.PReLU(),
                                    nn.Conv2d(self.mid_channels, out_channels,
                                              1))  # 为了恢复输出的通道数
