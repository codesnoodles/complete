...
class Model_cbigger_change(nn.Module):
    def __init__(self,num_channel=64,num_features=257):
        ...
        self.encoder = EncoderBlock_depth_Lpa(in_channels=3,out_channels=num_channel)
        self.neck1 =  CrossConformer_big_change(num_channel,num_channel)
        self.neck2 =  CrossConformer_big_change(num_channel,num_channel)
        self.decoder = ComplexDecoder_dep_Lpa()
        self.mask_decoder = MaskDecoder_dep_Lpa(num_features=num_features)
    def forward(self,x):
        # mag calculate
        mag = torch.sqrt(x[:,0,:,:]**2+x[:,1,:,:]**2).unsqueeze(1)
        phase = torch.angle(torch.complex(x[:,0,:,:,],x[:,1,:,:])).unsqueeze(1)
